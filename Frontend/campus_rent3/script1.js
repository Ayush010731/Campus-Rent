...
async function loadAvailableItems(category = "") {
  ...
  let url = "https://campus-rent-jcul.onrender.com/api/items/?is_available=true";
...
async function loadUserProfile() {
  ...
  try {
    const response = await fetch(
      "https://campus-rent-jcul.onrender.com/api/accounts/profile/",
      {
        headers: {
          "Authorization": "Bearer " + token
        }
...
async function loadActiveRentals() {
  ...
  const response = await fetch(
    "https://campus-rent-jcul.onrender.com/api/rentals/renter/active/",
    {
      headers: {
        "Authorization": "Bearer " + token
      }
 ...
 data.forEach(rental => {
  container.innerHTML += `
    <div class="item-card">
      <div class="item-image">
        ${rental.image 
          ? `<img src="https://campus-rent-jcul.onrender.com${rental.image}" style="width:100%;height:100%;object-fit:cover;">`
          : "📦"}
      </div>
      <div class="item-details">
        <h3>${rental.item}</h3>
...
async function loadCategoryItems(location, category, search) {
  ...
  data.forEach(item => {
    container.innerHTML += `
      <div class="item-card">
        <img src="https://campus-rent-jcul.onrender.com${item.image}">
        <h3>${item.title}</h3>
        <p>₹${item.price}/mo</p>
      </div>
    `;
